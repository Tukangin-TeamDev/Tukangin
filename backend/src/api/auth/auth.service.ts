...
export const loginUser = async (email: string, password: string) => {
  try {
    ...
    if (user.authProvider === AuthProvider.GOOGLE) {
      throw new AppError('Akun ini menggunakan login dengan Google. Silakan login dengan Google.', 400);
    }
 ...
 */
export const generateAccessToken = (userId: number, email: string, role: UserRole | string): string => {
